...
4. The diarized transcript is fed to a local Ollama instance (default model `gemma3:e4b`) to draft a structured minutes report: 會議紀要 (summary) · 主要結論 (conclusions) · 議題與要點 (topics & points) · 待辦行動項 (action items).
...
- `/api/report` POSTs the transcript to `${OLLAMA_URL}/api/chat` with `${OLLAMA_MODEL}` (default `gemma3:e4b`). The prompt explicitly names the target language so the report is written in the right script.
